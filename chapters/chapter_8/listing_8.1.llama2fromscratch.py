...
def get_batches(
    data, split, batch_size, context_window, config=MASTER_CONFIG
):
    ...
    ix = torch.randint(
        0, batch_data.size(0) - context_window - 1, (batch_size,)
    )
    x = torch.stack([batch_data[i : i + context_window] for i in ix]).long()
    y = torch.stack(
        [batch_data[i + 1 : i + context_window + 1] for i in ix]
    ).long()
    return x, y


MASTER_CONFIG.update({"batch_size": 8, "context_window": 16})
...
print(
    [
        (decode(xs[i].tolist()), decode(ys[i].tolist()))
...
assert (
    torch.linalg.norm(torch.arange(5).float())
    == (torch.arange(5).float() ** 2).sum() ** 0.5
)
rms = torch.linalg.norm(torch.arange(5).float()) * (
    torch.arange(5).numel() ** -0.5
)
assert torch.allclose(
    torch.linalg.norm(torch.arange(5).float() / rms), torch.tensor(5**0.5)
)
ff_rms = (
    torch.linalg.norm(batch, dim=(1, 2)) * batch.shape[1:].numel() ** -0.5
)
...
assert torch.allclose(
    torch.linalg.norm(ffx, dim=(1, 2)) ** 2, torch.tensor(143).float()
)
...
assert torch.allclose(
    q.transpose(0, 1)[0] @ layer.R[0], q[:, 0, :] @ layer.R[0]
)
...
class RoPEMaskedMultiheadAttention(nn.Module):
    def __init__(self, config):
        ...
        self.heads = nn.ModuleList(
            [
                RoPEMaskedAttentionHead(config)
    ...
    return sorted(
        [
            (
                name,
                100.0
                * float(torch.sum(torch.abs(param) <= tol))
...
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
    llama_optimizer, 300, eta_min=1e-5
)
