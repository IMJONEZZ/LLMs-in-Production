...
os.environ['OPENAI_API_BASE'] = 'http://0.0.0.0:1234/v1' #Replace with your server's address and port
os.environ['OPENAI_API_HOST'] = 'http://0.0.0.0:1234' #Replace with your host IP
...
vectorDB = load_dataset("csv", data_files="your dataset with embeddings.csv", split='train')
try:
    vectorDB.load_faiss_index('embeddings', "my_index.faiss")
except:
    print("No faiss index, run vectorDB.add_faiss_index(column='embeddings') and vectorDB.save_faiss_index('embeddings', 'my_index.faiss')")
message_history = []
...
num_tokens = len(tiktoker.encode(f"{prompt_template},\n" + message_history + query))
